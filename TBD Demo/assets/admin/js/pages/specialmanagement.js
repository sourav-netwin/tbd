...
$('body').on('click','.approve-sp-all', function(e){
	...
	if(c){
		...
		loading();
		$.ajax({
			url: base_url+'admin/specialmanagement/main_approve_all_special',
			type: 'POST',
			dataType: 'json',
			data: {
				sel_id: sel_id
			...
			success: function(data){
				var total_count_get = data.allStores.length;
						var processed_count = 1;
						var success_count = 0;
						
						$('#loading-text-dyn').html('<div class="new-loader">'+
									'<div class="left-text"><div class="fa fa-refresh fa-spin"></div><h3>THIS PROMOTION IS BEING ADDED TO</h3>'+
									'<div id="loading-text-dyn" style="position:relative;" class="outer-progress">'+ 
									'<div id="progress-colorr"></div></div>'+
										'<h2 id="loader_sub_text"></h2>	'+
									'</div><div class="numbers">'+
										'<span class="plain-text"></span>'+
										'<span class="number-bg"></span>'+
									'</div></div>');
						$.each(data.allStores,function(index, value){
							var complete_per = (processed_count/total_count_get)*100;
							$('#progress-colorr').css("width", complete_per+'%');
							$('#loader_sub_text').html(value.StoreName);
							$('.plain-text').html(processed_count);
							$('.number-bg').html(total_count_get);
							...
							/*$('#loading-text-dyn').html('<div class="new-loader">'+
									...
									'<div id="loading-text-dyn" style="position:relative;" class="outer-progress"> <div id="progress-colorr" style="width: '+complete_per+'%"></div></div>'+
										'<h2 id="loader_sub_text">'+value.StoreName+'</h2>	'+
									'</div><div class="numbers">'+
							...
							*/
								...
								dataType: 'json',
								data: {
									storeId: value.Id,sel_id:sel_id,special_product_count:data.special_product_count,storeNumber:index,storeCount:data.storeCount
								},
								success: function(data){
