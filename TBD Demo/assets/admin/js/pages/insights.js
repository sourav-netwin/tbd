...
$(function(){
	base_url = $('#base_url').val();
        ...
        // Show Specials(6 Month)
        if($('.knob1').length > 0){
            $.ajax({
                    url: base_url+'admin/insights/get_specials',
                    type: 'POST',
                    dataType: 'json',
                    data:{},
                    success: function(data){
                       if(data.result == 1){
                           ...
                           $.each(data.message, function(index, element) {
                                 //monthlyViews.push(element.specials_count)
                                 specialsData.push( {year: element.monthYear, value: element.specials_count});
                            });
	...
	if($('#view_graph').length > 0){
        ...
        // Show Visitors reports
        if($('#world-map-markers').length > 0){
            $.ajax({
                    url: base_url+'admin/insights/get_visitors',
                    type: 'POST',
                    dataType: 'json',
                    data:{},
                    success: function(data){
                       if(data.result == 1){
                           var visitorsData = {};
                           $.each(data.message, function(index, element) {
                                 visitorsData[element.stateCode] = element.users_count;
                            });
                            
                            $('#totalUsers').html(data.totalUsers);
                            $('#visitString').html(data.visitString);
                            ...
                            // Show data on Map
                                //var cdata = {"Western-Cape": "35", "KwaZulu-Natal": "15", "Orange-Free-State": "17", "Eastern-Cape": "20", "Gauteng": "10",  "Mpumalanga": "11", "Northern-Cape": "12", "Limpopo": "13", "North-West": "14"}    
                                $('#world-map-markers').vectorMap({
                                        ...
                                        onRegionLabelShow: function(event, label, code){
                                            //label.html(label.html() +' (' + code.toString() + ') <br>Count: ' + visitorsData[code]);
                                            var userCount = 0;
                                            if (typeof visitorsData[code] != "undefined") {
                                                userCount = visitorsData[code];        
                                            }
                                            label.html(label.html() +' : ' + userCount);
                                        }
                                        /*
                                        onRegionLabelShow: function(event, label, code){
                                        var labelSlug = label.html().replace(" ", "-");
                                        label.html(label.html() +' : ' + cdata[labelSlug] );
                                        }
                                        */
                                });
            ...
            $.ajax({
                    ...
                    success: function(data){
                            ...
                            if(data.result == 1){
                                    ...
                                    var progressClass= "";
                                        progressClass = 'progress-bar-graph-red'; // Make all progress bar red
                                    $.each(data.message, function(index, element) {
                                        /*
                                        switch (index) { 
                    ...
                    success: function(data){
                       if(data.result == 1){
                           ...
                           $.each(data.message, function(index, element) {
                            ...
                            var chart = new Chart(ctx).Line(salesChartData, salesChartOptions);

                            canvas.onclick = function (evt) {
                                var activePoints = chart.getPointsAtEvent(evt);
                                var arrIndex = chart.datasets[0].points.indexOf(activePoints[0]);
                                var monthName = months[arrIndex];
                                var monthlyViewsCount = monthlyViews[arrIndex];
                                
                                //alert("monthName : " + monthName + ", monthlyViews :" + monthlyViewsCount);
                                if(monthName){
                                    
                                
                                    $.ajax({
                                            url: base_url+'admin/insights/get_product_view_details',
                                            type: 'POST',
                                            dataType: 'json',
                                            data:{ monthName : monthName , monthlyViewsCount: monthlyViewsCount},
                                            success: function(data){
                                                    if(data.result == 1){
                                                            var showHtml ="<br>";
                                                            var index = parseInt(1);
                                                            ...
                                                            $('#all-category-view-title').html("Category wise product views : " + data.monthName);    
                                                            $('#all-category-view-expansion').html(showHtml);    
                                                    }else{
                                                        $('#all-category-view').show();
                                                        $('#all-category-view-title').html("Category wise product views : " + data.monthName);    
                                                        $('#all-category-view-expansion').html("<span class='error-message'>No product views for the selected month.<span><br>");    
                                                    }
                                            }
                                    });
                                } //if(monthName)    
				...
				if(data.result == 1){
					$.each(data.message, function(index1, element1) {
						...
						//$('#main_expansion').append('<div class="col-sm-6"><span class="t_sm_count"></span><div class=" text-center product_expansion_donut" id="'+html_id+'" style="height: 200px;padding-left: 0px;"></div></div>');
                                                $('#main_expansion').append('<div class="col-sm-12"><span class="t_sm_count"></span><div class=" text-center product_expansion_donut" id="'+html_id+'" style="height: 200px;padding-left: 0px;"></div></div>');
