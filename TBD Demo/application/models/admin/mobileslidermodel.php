...
class Mobileslidermodel extends CI_Model {
    ...
    public function get_sliders() {
        $this -> db -> select("Id, Image, Text, Color, BgColor, IsActive, Sequence")
            -> from('mobileslider')
            -> where(array(
                'IsRemoved' => 0
                )
    ...
    public function get_sequence() {
        ...
        $this -> db -> limit(1);
        $query = $this -> db -> get('mobileslider');
    ...
    public function add_slider($data) {
        ...
        $this -> db -> insert('mobileslider', $data);
    ...
    public function delete_slider($slider_id) {
        ...
        $this -> db -> update('mobileslider', $data);
    ...
    public function update_slider_sequence($id, $type, $update_array) {
        foreach ($update_array as $val) {
            $val_arr = explode(':', $val);
            $this -> db -> update("mobileslider", array("Sequence" => $val_arr[1]), array("Id" => $val_arr[0]));
        }
