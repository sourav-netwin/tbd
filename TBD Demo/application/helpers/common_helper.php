...
function custom_create_push_message($product_id = '', $retailer_id = '', $store_type_id = '', $store_id = '', $product_name = '', $retailer_name = '', $store_name = '', $store_type_name = '', $special_count = 0, $special_price = 0, $special_name = '', $success_count = 0, $store_array = array(), $product_array = array(), $store_detail_array = array(),  $specialFrom  ="", $specialTo ="",$special_id=0,$user_ids) {
    ...
    if (!empty($user_ids)) {//If users available to send the notifications
        ...
        foreach ($user_ids as $user_id) {
            ...
            if ($user_id['PrefLatitude'] && $user_id['PrefLongitude'] && $user_id['PrefDistance']) {
				...
				//print_r($store_detail_array);exit;
                $user_notification_settings = $CI -> specialproductmodel -> get_user_notification_settings($user_id['UserId']);
                if ($user_notification_settings && $user_notification_settings['PriceWatch'] == 1) {
                        ...
                        if ($is_pricewatch_alert_available) {
                            $price_watch_array[$user_id['UserId']][$product['id']][] = array(                                  
									'title' => $product['name'],
                                    'message' =>  $store_detail_array[$index]['retailerName'] . ' - ' . $store_detail_array[$index]['name'] ,
                ...
                if (!empty($store_det)) {
                    foreach ($store_det as $store_de) {
						...
						$storesDistance[$user_id['UserId']][$store_de['Id']]=array(
																			'StoreName' => $store_de['StoreName'],
																			'distance' => $store_de['distance'],
																		);
                    }
                }
       //  echo'<pre>';print_r($store_det);echo'</pre>';exit;
                if ($store_name && $store_get_id && !in_array($user_id['UserId'], $users_have_store)) {                                 
                        //Adding the user devices, in which they have the stores added in DB
                        if (!empty($special_user_list) && isset($special_user_list[$user_id['UserId']])) {                            
            ...
            if (!isset($users_have_store[$user_id['UserId']]) && !empty($comp_available_stores)) {
                   if(!isset($user_notification_settings))
					   $user_notification_settings = $CI -> specialproductmodel -> get_user_notification_settings($user_id['UserId']);
                    
                    if ($user_notification_settings && $user_notification_settings['Specials'] == 1) {  
							$user_preferred_brands = $CI -> specialproductmodel -> get_preferred_brands_for_users($user_id['UserId']);
                            ...
                            if (!array_key_exists($store_detail_array[$index]['id'] . ':' . $user_get['UserId'], $retailer_special_message)) {
                                if (!isset($retailer_count_arr[$user_get['UserId']])) {
                                    ...
                                    {   
                                        ...
                                        # Get stores names
                                        $store_names[$user_get['UserId']][$store_detail_array[$index]['id']]=$store_detail_array[$index]['name'];
                                    }
...
//                                    'title' => 'Special Added',
				...
				$numItems = count($price_change_array);
				$i = 0;
                if (!empty($price_change_array)) {
                    foreach ($price_change_array as $price_user_id => $price_array) {
							...
							if(!in_array($price_user_id,$price_change_alert_taken)){
								$multiple_single_insert[] = array(
									'Title' => $price_array['title'],
									'Message' => $price_array['message'],//$finalMessage,
							...
							} $price_change_alert_taken[]=$price_user_id;
                ...
                $retailer_final_arrayinc=0;
                if (!empty($retailer_final_array)) {
                    foreach ($retailer_final_array as $special_user_id => $special_array) {
                        if(isset($storesDistance[$special_user_id])){                            
                            $userStores = $storesDistance[$special_user_id];
							$finalStoreMessage= $generaLMessage= $special_name ." Special from ".date("d M", strtotime($specialFrom))." to ".date("d M", strtotime($specialTo)).' In Following Stores. '; $i=0;
							$storesIds=array();
							
                            foreach($userStores as $k=>$userStore)
                            {   
								$storeMsg = explode("Stores from",$special_array['message']);
								$storesIds[$k]=$userStore['distance'];
                                $msgTitleArr = explode("-",$special_array['title']);
                                $title = trim($msgTitleArr[0])." - ".$userStore['StoreName'];
								if($i<=5)
									$finalStoreMessage.=$title.', ';
								$i++;
                                
                            } //  foreach($userStores as $userStore)
							$finalStoreMessage=rtrim($finalStoreMessage,', ');
							$multiple_special_insert = array();
							$multiple_special_insert[] = array(
								'Title' => 'Special Added',
								...
								'CreatedOn' => date('Y-m-d H:i:s'),
								'specialId' => $special_id,
								'notificationType' => 'retailerStore',
								'storesIds' => serialize($storesIds),
								'generaLMessage' => $generaLMessage,
								
							);
							$special_array['title']='Special Added';
            ...
            foreach ($users_have_store as $us_st) {
                $multiple_insert[] = array(
					...
					'storesIds' => '',
					'generaLMessage' => $notification_array['message'],
                );
            }			
		...
		//print_r($storesDistance);
        if (!empty($price_watch_array)) {
         
			foreach ($price_watch_array as $pricewatch_user_id => $priceWatchArray) {   			
				foreach($priceWatchArray as $productId=>$perProductInfo){
					$message='';
					$notify_singlePriceWatchInfo=array();$store_ids='';$fullstoresIds=array();
					 $insert_info = array();$i=0;
				//	 print_r($storesDistance[$pricewatch_user_id]);
					// print_r($perProductInfo);
					foreach($perProductInfo as $singlePriceWatchInfo)
                    {
						...
						$retailer_id=$singlePriceWatchInfo['retailer_id'];						
						$store_type_id=$singlePriceWatchInfo['store_type_id'];
						if($i<=5)
							$store_ids.=$singlePriceWatchInfo['store_id'].',';
						if(isset($storesDistance[$pricewatch_user_id][$singlePriceWatchInfo['store_id']]))
						$fullstoresIds[$singlePriceWatchInfo['store_id']]=$storesDistance[$pricewatch_user_id][$singlePriceWatchInfo['store_id']]['distance'];
						$is_special=$singlePriceWatchInfo['is_special'];
						$is_location_message=$singlePriceWatchInfo['is_location_message'];
						$is_location_near_message=$singlePriceWatchInfo['is_location_near_message'];
						$i++;
						$SpecialPrice=$singlePriceWatchInfo['SpecialPrice'];
					}
					$finalMessage=$generaLMessage=$title.' is on special from '.$specialFrom.' to '.$specialTo.' @ just R'.$SpecialPrice.'';
				//	$finalMessage.=' in Following Stores '.$message;
					asort($fullstoresIds);
					//print_r($fullstoresIds);exit;
					$insert_info[] = array(
                            ...
                            'CreatedOn' => date('Y-m-d H:i:s'),
							'specialId' => $special_id,
							'notificationType' => 'priceWatch',
							'storesIds' => serialize($fullstoresIds),
							'generaLMessage' => $generaLMessage,
                        );
						
					$notify_singlePriceWatchInfo['title']='Price Watch Alert';
					$notify_singlePriceWatchInfo['message']=$finalMessage ;
					$notify_singlePriceWatchInfo['product_id']=$productId ;
					$notify_singlePriceWatchInfo['retailer_id']=$retailer_id ;
					$notify_singlePriceWatchInfo['store_type_id']=$store_type_id ;
					$notify_singlePriceWatchInfo['store_id']=$store_ids ;
					$notify_singlePriceWatchInfo['is_special']=$is_special ;
					$notify_singlePriceWatchInfo['is_location_message']=$is_location_message ;
					$notify_singlePriceWatchInfo['is_location_near_message']=$is_location_near_message ;
					
					send_push_notification($notify_singlePriceWatchInfo, array($pricewatch_user_id), $insert_info);                 
				}
			}
...
function create_push_notification_message($product_id = '', $retailer_id = '', $store_type_id = '', $store_id = '', $product_name = '', $retailer_name = '', $store_name = '', $store_type_name = '', $special_count = 0, $special_price = 0, $special_name = '', $success_count = 0, $store_array = array(), $product_array = array(), $store_detail_array = array(),  $specialFrom  ="", $specialTo ="")
{
    ...
    $user_have_price_alert = array();
    $price_change_array = array();
    $price_change_count = array();
    $retailer_special_message = array();
    $retailer_count_arr = array();
    $user_add = array();
    $comp_available_stores = array();
    $product_names = array(); // Array to store product names
    $productNameMessage = "";
    $store_names = array(); // Array to store name of the stores
    $storeNameMessage = "";
    $usersStoreIds = array();
                    ...
                    if ($user_notification_settings['PriceWatch'] == 1) {
                        ...
                        if ($is_pricewatch_alert_available) {                            
                               ...
                               $price_watch_array = array(
                                ...
                                send_push_notification($price_watch_array, array($user_id['UserId']), $insert_info);
...
if (!function_exists('gainLoyaltyPointsMailOfUser')) {

        function gainLoyaltyPointsMailOfUser($user_id,$event) {
            ...
            // Send Admin details regarding the user registration.
            $email_template_details = $CI -> emailtemplatemodel -> get_email_template_details(6);
            $emailBody = "";
            $emailBody = $email_template_details['Content'];
            $emailBody = str_replace("{USERNAME}", $user_details['FirstName'] . " " . $user_details['LastName'], $emailBody);
            $emailBody = str_replace("{USERID}", $user_details['Id'], $emailBody);
			$emailBody = str_replace("{EVENT}", $event, $emailBody);
            $emailBody = str_replace("{EMAIL}", $user_details['Email'], $emailBody);
            ...
            $CI -> email -> from($email_template_details['FromEmail']);
            $CI -> email -> to($email_template_details['ToEmail']);
            $CI -> email -> subject("The Best Deals - User Gain Loyalty Point(s)");
