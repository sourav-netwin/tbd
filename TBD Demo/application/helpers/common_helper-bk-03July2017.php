...
function send_save_actual_notification($token_set,$notification_array,$users)
{
    ob_start();
			set_time_limit(0);
...
function send_push_notification($notification_array, $users, $insert_data = array()) {
	...
	//print_r($notification_array);	print_r($users);	print_r($device_token_array);
    ...
    if (isset($device_token_array[0])) {
        ...
        foreach ($device_token_array as $token_set) {
			...
			//print_r($inc);
			////	sleep(10); }
			send_save_actual_notification($token_set,$notification_array,$users);
   ...
   */
    ...
    if (!empty($user_ids)) {//If users available to send the notifications
        foreach ($user_ids as $user_id) {
          //if( $user_id['UserId'] == 77 || $user_id['UserId'] == 201 || $user_id['UserId'] == 217){
           //if( $user_id['UserId'] == 77){ 
            $comp_available_stores = [];
            if ($user_id['PrefLatitude'] && $user_id['PrefLongitude'] && $user_id['PrefDistance']) {
                ...
                echo "<pre>";
                //print_r($store_det);
                echo "<br>store_name : ".$store_name;
                echo "<br>$store_get_id : ".$store_get_id;
                exit;
                */
             ...
             //   exit;
              ...
              //  echo $this->db->last_query();
                ...
                /*
                echo "<pre>";
                print_r($usersStoreIds);
                print_r($comp_available_stores);
                exit;
                */
                        ...
                        if (!empty($special_user_list)) {
                            if (isset($special_user_list[$user_id['UserId']])) {
                               // $special_user_list[$user_id['UserId']]['StoreId'];
                                ...
                                //if (($special_user_list[$user_id['UserId']]['Specials'] == 1 || $special_user_list[$user_id['UserId']]['PreferredStoreOnly'] == 1)) {
                                    ...
                                    $users_have_store[$user_id['UserId']] = array(
                                        'UserId' => $user_id['UserId'],
                                        'StoreName' => $store_name,
                                        'StoreId' => $special_user_list[$user_id['UserId']]['StoreId']
                                    );
                                    */
            ...
            if (!isset($users_have_store[$user_id['UserId']])) {
                if (!empty($comp_available_stores)) {                    
                    ...
                    /*
                    echo "<pre>";
                    print_r($user_notification_settings);
                    print_r($user_preferred_brands);
                    exit;
                    */
        ...
        if (!empty($users_have_store)) { //if there are users with stores, then send push notification to them
            ...
            if (!empty($product_array)) {
                ...
                foreach ($product_array as $index => $product) {
                    ...
                    if( $prodCounter %20 == 0)
                    {
                        echo "<br>prodCounter :".$prodCounter;
                        sleep(5);
                    }
                    $prodCounter++;
                    */
...
//                                    'title' => 'Special Added',
                ...
                if (!empty($price_change_array)) {
                    foreach ($price_change_array as $price_user_id => $price_array) {
                        ...
                        //print_r('price_array');//print_r(array($price_user_id));
						$price_change_arrayinc++;
						//if($price_change_arrayinc%50==0) sleep(10);
                    ...
                    foreach ($retailer_final_array as $special_user_id => $special_array) {
                        if(isset($store_names[$special_user_id])){                            
                            ...
                            {   
								...
								$retailer_final_arrayinc++;
								//if($retailer_final_arrayinc%50==0) sleep(10);
       ...
       // //print_r( $price_watch_array);
        # Send Price Watch notification
		$price_watch_arrayinc=0;
            ...
            foreach ($price_watch_array as $pricewatch_user_id => $priceWatchArray) {                        
                if(isset($price_watch_array[$pricewatch_user_id])){                            
                    ...
                    { 
                        ...
                        # Send notification
						//print_r('singlePriceWatchInfo');
						$price_watch_arrayinc++;
						//if($price_watch_arrayinc%50==0) sleep(10);
